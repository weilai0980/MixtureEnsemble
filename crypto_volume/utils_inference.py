...
#                 g_src_sample]
    ...
    def bayesian_inference(self, 
        ...
        bayes_mean = bayes_mean*y_z
        bayes_var_total = bayes_var_total*(y_z**2)
        std_total_mean = np.mean(np.sqrt(bayes_var_total))
        nnllk = np.mean(-1.0*np.log( 1.0*np.mean(lk_sample, 0)/y_z + 1e-5 ))
        
        tmp_num = len(y_z)
        y_z_expand = np.reshape(y_z, [1, tmp_num, 1])
        m_src_sample = m_src_sample*y_z_expand
        v_src_sample = v_src_sample*(y_z_expand**2)
...
#         func_pred_interval_width(y_ori, yhat_low = y_low_total, yhat_up = y_up_total), 
        ...
        return [func_rmse(y_ori, bayes_mean),
